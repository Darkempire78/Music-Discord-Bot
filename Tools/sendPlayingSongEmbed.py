...
def sendPlayingSongEmbed(self, ctx, music):
    ...
    embed.add_field(name="Loop :", value=str(self.bot.music[ctx.guild.id]["loop"]).replace("True", f"{self.bot.emojiList.true}").replace("False", f"{self.bot.emojiList.false}"), inline=True)
    embed.add_field(name="Loop queue :", value=str(self.bot.music[ctx.guild.id]["loopQueue"]).replace("True", f"{self.bot.emojiList.true}").replace("False", f"{self.bot.emojiList.false}"), inline=True)
